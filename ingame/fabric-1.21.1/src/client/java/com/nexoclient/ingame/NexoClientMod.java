...
public final class NexoClientMod implements ClientModInitializer {
    ...
    private static boolean titleScreenFaulted;
    ...
    public void onInitializeClient() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (!titleScreenFaulted && client.currentScreen instanceof TitleScreen) {
                try {
                    client.setScreen(new NexaTitleScreen());
                }
                catch (RuntimeException | LinkageError error) {
                    titleScreenFaulted = true;
                    reportDisabled("portada principal", error);
                }
            }
