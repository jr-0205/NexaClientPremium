...
/// <summary>
/// Capacidades React que requieren servicios nativos. NEXA In-Game se consume
/// exclusivamente como artefacto ya publicado: este router nunca compila mods.
/// </summary>
internal sealed class NexaDesktopMessageRouter
{
    ...
    public NexaDesktopMessageRouter(NexoPaths paths, CoreWebView2 webView)
    {
        ...
        var localArtifactRoot = PrepareLocalArtifactRoot(paths, developmentRoot);
        inGame = new NexoInGameArtifactService(http, catalog, paths, localArtifactRoot);
    }
    ...
    private static string? FindRepositoryRoot()
    {
        foreach (var root in new[] { Environment.CurrentDirectory, AppContext.BaseDirectory })
        {
            ...
            {
                if (!Directory.Exists(Path.Combine(directory.FullName, "artifacts", "nexo-ingame"))) continue;
    ...
    private static string PrepareLocalArtifactRoot(NexoPaths paths, string? developmentRoot)
    {
