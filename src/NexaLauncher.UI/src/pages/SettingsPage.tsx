      ...
      <div className="settings-grid">
        ...
        <article className="settings-card glass-panel">
          ...
          <button className={`switch-row ${closeOnLaunch ? "enabled" : ""}`} type="button" onClick={() => setCloseOnLaunch((value) => !value)}>
            ...
            <span>
              <strong>Cerrar launcher al iniciar</strong>
              <small>{closeOnLaunch ? "NEXA se cerrará cuando Minecraft arranque." : "NEXA permanecerá abierto durante la sesión."}</small>
            </span>
        ...
        <article className="settings-card glass-panel settings-wide">
          <span className="eyebrow">NEXA IN-GAME</span>
          <h2>Distribución por builds publicadas</h2>
          <p>
            NEXA Client ya no compila mods localmente. El módulo In-Game se instalará únicamente desde JARs publicados,
            verificados por versión de Minecraft, loader y SHA-256.
          </p>
          <div className="trust-row">
            <ShieldCheck size={17} />
            <span>Sin Gradle, sin JDK de desarrollo y sin workspaces temporales dentro del launcher.</span>
          </div>
        </article>
